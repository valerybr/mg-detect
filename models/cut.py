...
class CycleCUTModel(nn.Module):
    ...
    def __init__(
        ...
        self.G_BA = init_weights(ResnetGenerator(
            in_channels=in_channels, out_channels=in_channels,
            n_blocks=n_blocks,
        )).to(device)

        self.D_A = init_weights(
        ...
        # ----- optimizers (G, D, F separate — matches CUT) -----
        ...
        self.opt_F = torch.optim.Adam(
            itertools.chain(self.mlps_AB.parameters(), self.mlps_BA.parameters()),
            lr=lr, betas=(beta1, 0.999),
        )

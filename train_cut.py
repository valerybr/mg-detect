...
Usage:
    python -m models.train_cut --config configs/base.yaml
    python -m models.train_cut --config configs/base.yaml train.lr=0.001
    python -m models.train_cut --config configs/base.yaml \\
        train.lr=0.001 model.ngf=32 output.dir=runs/exp2

All parameters are defined in configs/defaults.yaml.
...
_DEFAULTS = Path(__file__).parent / "configs" / "defaults.yaml"
...
def _validate(cfg: DictConfig):
    required = {"data.root": cfg.data.root,
...
def train(cfg: DictConfig):
    ...
    model = CycleCUTModel(
        ...
        lambda_cyc=cfg.train.lambda_cyc,
...
def main():
    ...
    parser.add_argument("--config", default=None,
                        help="Path to experiment YAML config (overrides defaults.yaml)")
